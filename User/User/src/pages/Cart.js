...
const Cart = () => {
  ...
  useEffect(() => {
    const amount = cart.reduce((acc, curr) => {
      const price = Number(curr.price.$numberDecimal || curr.price);
      console.log(`Item: ${curr.title}, Price: ${curr.price}, Converted Price: ${price}`);
      return acc + (isNaN(price) ? 0 : price);
    }, 0);
    console.log("Calculated Amount:", amount, typeof amount);
